# PathWise Narrative Profile System
...
When a user completes the PathWise assessment, they receive a 500–800 word second-person narrative that describes their career personality. This is the moment the product earns emotional trust — where raw scores become a story the user recognizes as their own. The narrative is assembled from modular paragraph templates selected by a deterministic algorithm keyed to Big Five trait scores, RIASEC interest dominance, values, environment preferences, and trajectory intent.

The design philosophy is direct: say something true about this specific person that they could not have written themselves. Generic affirmations do not accomplish this. Every paragraph must be specific enough that the reader thinks "how did it know that?" and too accurate to be shared with the wrong person.
...
interface NarrativeProfile {
  headline: string;           // "You are The Analytical Architect"
  tagline: string;            // one-line summary
  paragraphs: {
    opening: string;          // who you are at your core
    workStyle: string;        // how you approach work
    strengths: string;        // what you do better than most
    growthEdge: string;       // what to watch out for
    idealEnvironment: string; // where you thrive
    careerDirection: string;  // where you're heading
  };
  shareQuote: string;         // one sentence for social sharing
}

// Input types (mapped from existing AssessmentInput + bigFive scores)
...
interface ValuesProfile {
  primary: string[];   // top 3 values from assessment
  secondary: string[]; // next 2 values
}

interface EnvironmentPrefs {
  workMode: "remote" | "hybrid" | "onsite" | "flexible";
  teamSize: "solo" | "small" | "medium" | "large";
  pace: "fast" | "steady" | "variable";
  culture: string[]; // e.g. ["mentorship-heavy", "async", "collaborative"]
}
...
type Archetype = string; // e.g. "Analytical Architect", "Creative Builder"
```
...
### Section 1: Opening Paragraphs (Big Five — Openness)

The opening paragraph is selected when Openness is the highest or second-highest Big Five trait. Two variants prevent identical outputs for users with similar profiles.

#### High Openness — Variant A
...
### Section 2: Opening Paragraphs (Big Five — Conscientiousness)

The opening paragraph is selected when Conscientiousness is the highest or second-highest trait.

#### High Conscientiousness — Variant A
...
#### High Conscientiousness — Variant B
...
#### Low Conscientiousness — Variant A
...
#### Low Conscientiousness — Variant B
...
### Section 3: Opening Paragraphs (Big Five — Extraversion)

#### High Extraversion — Variant A
...
### Section 4: Opening Paragraphs (Big Five — Agreeableness)

#### High Agreeableness — Variant A
...
### Section 5: Opening Paragraphs (Big Five — Emotional Stability)

#### High Emotional Stability — Variant A
...
#### High Emotional Stability — Variant B
...
#### Low Emotional Stability (Moderate Neuroticism) — Variant A
...
#### Low Emotional Stability (Moderate Neuroticism) — Variant B
...
### Section 6: Work Style Paragraphs (RIASEC Dominant Types)

These paragraphs are selected based on the highest-scoring RIASEC dimension.
...
### Section 7: Strengths Paragraphs

Ten templates, each addressing a distinct combination. Selected based on the strongest trait combination derived from Big Five scores and RIASEC dominance.

#### Combination 1: Analytical + Creative
...
#### Combination 2: Empathetic + Organized
...
#### Combination 3: Visionary + Practical
...
#### Combination 4: Independent + Collaborative
...
#### Combination 5: Resilient + Ambitious
...
#### Combination 6: Detail-Oriented + Big-Picture
...
#### Combination 7: Creative + Systematic
...
#### Combination 8: People-Smart + Data-Smart
...
#### Combination 10: Adaptable + Principled
...
### Section 8: Growth Edge Paragraphs

Framed throughout as development opportunities, not deficiencies. Selected based on the most extreme or dominant trait.

#### Growth Edge 1: High Autonomy
...
#### Growth Edge 3: High Perfectionism
...
#### Growth Edge 4: High Risk Tolerance
...
#### Growth Edge 5: High Introversion
...
#### Growth Edge 6: High Agreeableness
...
#### Growth Edge 7: High Novelty-Seeking
...
#### Growth Edge 8: High Structure-Need
...
#### Growth Edge 9: High Competitiveness
...
### Section 9: Ideal Environment Paragraphs

Selected based on environment preference scores from the assessment.

#### Environment 1: Remote + Small Team + Fast Pace
...
#### Environment 2: Hybrid + Medium Team + Steady Pace
...
#### Environment 3: Onsite + Large Organization + Structured
...
#### Environment 4: Remote + Solo + Self-Directed
...
#### Environment 5: Hybrid + Small Team + Mentorship-Heavy
...
#### Environment 6: Onsite + Medium Team + Collaborative
...
#### Environment 7: Flexible + Portfolio/Freelance + Variable
...
#### Environment 8: Remote + Large Team + Async Communication
...
### Section 10: Career Direction Paragraphs

Selected based on the trajectory type submitted in the assessment.
 ...
 * generateNarrative
 *
 * Assembles a NarrativeProfile from scored assessment dimensions.
 * All paragraph selection is deterministic: same inputs produce same output.
 * Variant selection uses a hash of the userId to avoid duplicate-feeling
 * profiles for users with identical scores.
 */
function generateNarrative(
  ...
  environment: EnvironmentPrefs,
  trajectory: TrajectoryType,
  userId: string,
): NarrativeProfile {

  // ── Step 1: Determine variant seed (0 or 1) from userId hash ──────────
  // Ensures users with identical profiles still receive different variants,
  // preventing the "we got the same result" social sharing problem.
  const variantSeed = simpleHash(userId) % 2; // 0 or 1

  // ── Step 2: Rank Big Five traits ──────────────────────────────────────
  const bigFiveRanked = rankBigFive(bigFive);
  const primaryTrait = bigFiveRanked[0];   // highest scoring dimension
  const secondaryTrait = bigFiveRanked[1]; // second highest

  // ── Step 3: Select opening paragraph ─────────────────────────────────
  // Opening is driven by the two highest Big Five traits.
  // Primary trait determines the dimension; variant seed selects A or B.
  const opening = selectOpening(primaryTrait, bigFive, variantSeed);

  // ── Step 4: Select workStyle paragraph ───────────────────────────────
  // Dominant RIASEC type drives work style selection.
  const dominantRIASEC = getDominantRIASEC(riasec);
  const workStyle = RIASEC_PARAGRAPHS[dominantRIASEC];

  // ── Step 5: Select strengths paragraph ───────────────────────────────
  // Map primary + secondary trait combination to a strengths template.
  // Falls back to most relevant single-trait strengths if combination
  // is not explicitly mapped.
  const strengths = selectStrengths(primaryTrait, secondaryTrait, bigFive, riasec);

  // ── Step 6: Select growth edge paragraph ─────────────────────────────
  // Driven by the most extreme trait score (furthest from 50 in either
  // direction), weighted toward traits above 75 or below 25.
  const mostExtremeTrait = getMostExtremeTrait(bigFive);
  const growthEdge = selectGrowthEdge(mostExtremeTrait, bigFive, values);

  // ── Step 7: Select ideal environment paragraph ────────────────────────
  const idealEnvironment = selectEnvironment(environment);

  // ── Step 8: Select career direction paragraph ─────────────────────────
  const careerDirection = CAREER_DIRECTION_PARAGRAPHS[trajectory];

  // ── Step 9: Generate headline and tagline ─────────────────────────────
  const { headline, tagline } = generateHeadlineAndTagline(archetype, bigFive, riasec, trajectory);

  // ── Step 10: Generate share quote ─────────────────────────────────────
  const shareQuote = generateShareQuote(archetype, primaryTrait, dominantRIASEC);
...
function rankBigFive(scores: BigFiveScores): Array<keyof BigFiveScores> {
  return (Object.keys(scores) as Array<keyof BigFiveScores>)
    .sort((a, b) => scores[b] - scores[a]);
}

function getDominantRIASEC(riasec: RIASECScores): keyof RIASECScores {
  return (Object.keys(riasec) as Array<keyof RIASECScores>)
    .reduce((a, b) => riasec[a] > riasec[b] ? a : b);
}

function getMostExtremeTrait(bigFive: BigFiveScores): keyof BigFiveScores {
  // Trait furthest from the neutral midpoint of 50
  return (Object.keys(bigFive) as Array<keyof BigFiveScores>)
    .reduce((a, b) => Math.abs(bigFive[a] - 50) > Math.abs(bigFive[b] - 50) ? a : b);
}

function selectOpening(
  primaryTrait: keyof BigFiveScores,
  bigFive: BigFiveScores,
  variantSeed: number,
): string {
  ...
  // Map trait + direction to paragraph keys
  const key = `${primaryTrait}_${isHigh ? "high" : "low"}_${variantSeed === 0 ? "a" : "b"}`;
  return OPENING_PARAGRAPHS[key];
}

function selectStrengths(
  primaryTrait: keyof BigFiveScores,
  secondaryTrait: keyof BigFiveScores,
  bigFive: BigFiveScores,
  riasec: RIASECScores,
): string {
  // Combination matrix — maps trait pairs to strengths templates
  ...
  const mapped = combinationKey in traitCombinationMap
    ? traitCombinationMap[combinationKey]
    : inferStrengthsFromDominant(primaryTrait, bigFive, riasec);

  return STRENGTHS_PARAGRAPHS[mapped];
}

function inferStrengthsFromDominant(
  primaryTrait: keyof BigFiveScores,
  bigFive: BigFiveScores,
  riasec: RIASECScores,
): string {
  // Fallback: map single dominant trait to most relevant strengths template
...
function selectGrowthEdge(
  mostExtremeTrait: keyof BigFiveScores,
  bigFive: BigFiveScores,
  values: ValuesProfile,
): string {
  ...
  // Map extreme traits to growth edge keys
  const growthEdgeMap: Record<string, string> = {
    ...
    "emotionalStability_high":   "idealism",    // very high stability → may underweight emotional signals
    "emotionalStability_low":    "empathy",     // low stability → absorbs others' stress
  };
...
function selectEnvironment(prefs: EnvironmentPrefs): string {
  // Priority matching: work mode first, then team size, then pace
  if (prefs.workMode === "remote" && prefs.teamSize === "small")
    return ENVIRONMENT_PARAGRAPHS["remote_small_fast"];
  if (prefs.workMode === "hybrid" && prefs.pace === "steady")
    return ENVIRONMENT_PARAGRAPHS["hybrid_medium_steady"];
  if (prefs.workMode === "onsite" && prefs.teamSize === "large")
    return ENVIRONMENT_PARAGRAPHS["onsite_large_structured"];
  if (prefs.workMode === "remote" && prefs.teamSize === "solo")
    return ENVIRONMENT_PARAGRAPHS["remote_solo_selfdirected"];
  if (prefs.workMode === "hybrid" && prefs.culture.includes("mentorship-heavy"))
    return ENVIRONMENT_PARAGRAPHS["hybrid_small_mentorship"];
  if (prefs.workMode === "onsite" && prefs.teamSize === "medium")
    return ENVIRONMENT_PARAGRAPHS["onsite_medium_collaborative"];
  if (prefs.workMode === "flexible")
    return ENVIRONMENT_PARAGRAPHS["flexible_portfolio_variable"];
  if (prefs.workMode === "remote" && prefs.teamSize === "large")
    return ENVIRONMENT_PARAGRAPHS["remote_large_async"];

  // Default: hybrid medium steady — covers the broadest set of unmatched preferences
  return ENVIRONMENT_PARAGRAPHS["hybrid_medium_steady"];
}

function generateHeadlineAndTagline(
  archetype: Archetype,
  bigFive: BigFiveScores,
  riasec: RIASECScores,
  trajectory: TrajectoryType,
): { headline: string; tagline: string } {
  const headline = `You are The ${archetype}`;

  // Tagline constructed from dominant RIASEC + trajectory
  const dominantRIASEC = getDominantRIASEC(riasec);
  const riasecDescriptors: Record<keyof RIASECScores, string> = {
  ...
  const trajectoryDescriptors: Record<TrajectoryType, string> = {
  ...
  const tagline = `Someone who ${riasecDescriptors[dominantRIASEC]} — ${trajectoryDescriptors[trajectory]}.`;

  return { headline, tagline };
}

function generateShareQuote(
  archetype: Archetype,
  primaryTrait: keyof BigFiveScores,
  dominantRIASEC: keyof RIASECScores,
): string {
  const traitPhrases: Record<keyof BigFiveScores, string> = {
  ...
  return `"${traitPhrases[primaryTrait]}." — My PathWise career profile (The ${archetype})`;
}
...
The paragraphs above are referenced by the algorithm through the following lookup objects. These should be maintained in a separate `narrative-templates.ts` file and imported by the generation function.

```typescript
// narrative-templates.ts

export const OPENING_PARAGRAPHS: Record<string, string> = {
  openness_high_a:             "/* High Openness Variant A */",
  openness_high_b:             "/* High Openness Variant B */",
  openness_low_a:              "/* Low Openness Variant A */",
  openness_low_b:              "/* Low Openness Variant B */",
  conscientiousness_high_a:    "/* High Conscientiousness Variant A */",
  conscientiousness_high_b:    "/* High Conscientiousness Variant B */",
  conscientiousness_low_a:     "/* Low Conscientiousness Variant A */",
  conscientiousness_low_b:     "/* Low Conscientiousness Variant B */",
  extraversion_high_a:         "/* High Extraversion Variant A */",
  extraversion_high_b:         "/* High Extraversion Variant B */",
  extraversion_low_a:          "/* Low Extraversion Variant A */",
  extraversion_low_b:          "/* Low Extraversion Variant B */",
  agreeableness_high_a:        "/* High Agreeableness Variant A */",
  agreeableness_high_b:        "/* High Agreeableness Variant B */",
  agreeableness_low_a:         "/* Low Agreeableness Variant A */",
  agreeableness_low_b:         "/* Low Agreeableness Variant B */",
  emotionalStability_high_a:   "/* High Emotional Stability Variant A */",
  emotionalStability_high_b:   "/* High Emotional Stability Variant B */",
  emotionalStability_low_a:    "/* Low Emotional Stability Variant A */",
  emotionalStability_low_b:    "/* Low Emotional Stability Variant B */",
};

export const RIASEC_PARAGRAPHS: Record<keyof RIASECScores, string> = {
  realistic:     "/* Realistic work style */",
  investigative: "/* Investigative work style */",
  artistic:      "/* Artistic work style */",
  social:        "/* Social work style */",
  enterprising:  "/* Enterprising work style */",
  conventional:  "/* Conventional work style */",
};

export const STRENGTHS_PARAGRAPHS: Record<string, string> = {
  analytical_creative:       "/* Analytical + Creative */",
  empathetic_organized:      "/* Empathetic + Organized */",
  visionary_practical:       "/* Visionary + Practical */",
  independent_collaborative: "/* Independent + Collaborative */",
  resilient_ambitious:       "/* Resilient + Ambitious */",
  detail_bigpicture:         "/* Detail + Big Picture */",
  creative_systematic:       "/* Creative + Systematic */",
  people_data:               "/* People-smart + Data-smart */",
  patient_driven:            "/* Patient + Driven */",
  adaptable_principled:      "/* Adaptable + Principled */",
};

export const GROWTH_EDGE_PARAGRAPHS: Record<string, string> = {
  autonomy:       "/* High autonomy growth edge */",
  empathy:        "/* High empathy growth edge */",
  perfectionism:  "/* High perfectionism growth edge */",
  risk_taking:    "/* High risk-taking growth edge */",
  introversion:   "/* High introversion growth edge */",
  agreeableness:  "/* High agreeableness growth edge */",
  novelty_seeking:"/* High novelty-seeking growth edge */",
  structure_need: "/* High structure-need growth edge */",
  competitiveness:"/* High competitiveness growth edge */",
  idealism:       "/* High idealism growth edge */",
};

export const ENVIRONMENT_PARAGRAPHS: Record<string, string> = {
  remote_small_fast:            "/* Remote + Small + Fast */",
  hybrid_medium_steady:         "/* Hybrid + Medium + Steady */",
  onsite_large_structured:      "/* Onsite + Large + Structured */",
  remote_solo_selfdirected:     "/* Remote + Solo + Self-directed */",
  hybrid_small_mentorship:      "/* Hybrid + Small + Mentorship */",
  onsite_medium_collaborative:  "/* Onsite + Medium + Collaborative */",
  flexible_portfolio_variable:  "/* Flexible + Portfolio + Variable */",
  remote_large_async:           "/* Remote + Large + Async */",
};

export const CAREER_DIRECTION_PARAGRAPHS: Record<TrajectoryType, string> = {
...
Every paragraph in this system must meet the following tests before it ships.

**Voice and register.** Second person throughout. Write as a trusted senior colleague who knows the reader better than the reader expected — not as a performance review, not as a therapist, and not as a chatbot completing a request. Authoritative and warm simultaneously.

**Specificity test.** If a sentence could appear unchanged in a profile for someone with the opposite score, rewrite it. The high-agreeableness paragraph must be visibly different from the low-agreeableness paragraph. The Artistic work style paragraph must describe something meaningfully different from the Social one. Vague affirmations do not pass this test.

**Jargon floor.** No psychological terminology visible to the reader. Do not say "your openness score" or "your Big Five profile." The underlying model should be invisible; the output should feel like intuition captured in language.

**Grade level.** A 20-year-old should understand every sentence. No multi-clause academic constructions. No nominalizations where a verb will do. If a paragraph feels like it belongs in a research paper, simplify it until it does not.

**Positive framing without dishonesty.** Growth edges are framed as development opportunities, not criticisms — but they must still be honest. A growth edge paragraph that hedges so heavily it says nothing is as useless as one that is simply unkind. Name the real risk, then frame the path forward.

**Word count discipline.** Each paragraph should run 60–120 words. The full assembled narrative targets 500–800 words. If the narrative runs long, trim from the strengths and growth edge sections first — these are the most likely to contain redundancy with the opening.

**The social sharing test.** Read the `shareQuote` aloud. Would a real person send this to a friend? Would it survive a Twitter character count and still be interesting? If it sounds like a corporate tagline, rewrite it as something a person would actually say about themselves.

---

## Integration Notes

The `generateNarrative` function should be added to the assessment backend as a new export in `career-brain.ts` or a dedicated `narrative-engine.ts` file in `/home/admin1/PathWise/backend/assessment/`. It receives the existing `AssessmentInput` structure plus derived Big Five and RIASEC scores, which will need to be computed either as part of the assessment questionnaire answer processing or as a separate scoring step run at result generation time.

The `NarrativeProfile` interface should be added to the `AssessmentResult` interface in both `assessment.ts` and `src/types/index.ts`, and stored as a JSON column in the `assessments` table (migration required). On the frontend, the narrative display component should render the six paragraphs in sequence with the headline and tagline above and the share quote available as a one-click copy action below.
